...
function ServiceShowcase({ data }) {
    return (
       data&& <div className='font-mont mt-28'>
            <h1 className='text-5xl text-center mb-6 font-bold '>{data?.title}</h1>
            <div className='grid grid-cols-3 gap-y-10 gap-x-3 mx-40'>
                {data?.elements?.map((elem, index) => (
                    <div className={`relative flex justify-center items-center flex-col ${index % 3 === 1 ? 'mt-5' : ''}`} key={elem.title}>
                        ...
                        <h5 className='font-bold text-2xl mt-1'>{elem.title}</h5>
                    </div>
                ))}
            </div>
        </div>
